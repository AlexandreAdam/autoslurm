import pytest
from unittest.mock import patch, MagicMock, Mock
from autoslurm.run_slurm import get_job_id_from_sbatch_output
from autoslurm.run_slurm import (
    run_slurm_remotely,
    run_slurm_locally,
    run_bulk_submit_driver_remotely,
)
import os
import json


@pytest.fixture
def mock_ssh():
    """Sets up a mock for subprocess.run for testing purposes."""
    with patch("subprocess.run") as mock_run:
        mock_run.return_value = MagicMock(
            returncode=0, stdout="Submitted batch job 12345\n", stderr=""
        )
        yield mock_run


@pytest.fixture
def mock_load_config(tmp_path):
    mock_config = {"local": {"path": tmp_path}}
    os.makedirs(tmp_path / "jobs", exist_ok=True)
    with patch(
        "autoslurm.run_slurm.load_config", return_value=mock_config
    ) as mock_load_config:
        yield mock_load_config


def test_get_job_id_from_sbatch_output():
    output = "Submitted batch job 12345"  # Simulation of SLURM output
    job_id = get_job_id_from_sbatch_output(output)
    assert job_id == "12345"


def test_get_job_id_from_sbatch_output_failure():
    output = "Error: something went wrong"
    with pytest.raises(ValueError):
        get_job_id_from_sbatch_output(output)


def test_run_slurm_remotely(mock_ssh, mock_load_config):
    # Ensure that read() returns bytes
    mock_machine_config = {
        "hostname": "testhost",
        "username": "testuser",
        "key_path": "testkey",
        "path": "/path/to/remote",
    }

    # Call the function
    job_id = run_slurm_remotely("remote_script.sh", machine_config=mock_machine_config)

    assert job_id == "12345"


def test_run_slurm_remotely_probes_remote_root_when_path_missing(
    monkeypatch, mock_load_config
):
    calls = []

    def fake_run(cmd, *args, **kwargs):
        calls.append(cmd)
        if cmd[0] == "ssh" and cmd[-1].startswith("bash -lc "):
            return MagicMock(returncode=0, stdout="/remote/autoslurm\n", stderr="")
        if cmd[0] == "ssh":
            return MagicMock(returncode=0, stdout="Submitted batch job 12345\n", stderr="")
        raise AssertionError(f"Unexpected command: {cmd}")

    monkeypatch.setattr("subprocess.run", fake_run)

    mock_machine_config = {
        "hostname": "testhost",
        "username": "testuser",
        "key_path": "testkey",
        "env_command": "source /path/to/venv/bin/activate",
    }

    job_id = run_slurm_remotely("remote_script.sh", machine_config=mock_machine_config)

    assert job_id == "12345"
    sbatch_calls = [cmd for cmd in calls if cmd[0] == "ssh" and cmd[-1].startswith("sbatch ")]
    assert sbatch_calls
    assert "/remote/autoslurm/slurm/remote_script.sh" in sbatch_calls[0][-1]


@patch("subprocess.run")
def test_run_slurm_locally(mock_run, mock_load_config):
    # Setup mock behavior
    mock_run.return_value = Mock(stdout="Submitted batch job 67890")

    # Call the function
    job_id = run_slurm_locally("local_script.sh")

    assert job_id == "67890"
    # Same as test above
    # mock_run.assert_called_with(['sbatch', 'local_script.sh'], capture_output=True, text=True)


@patch("subprocess.run")
def test_run_bulk_submit_driver_remotely(mock_run, mock_load_config):
    response = {
        "ok": True,
        "job_ids": {"A": "1", "B": "2"},
        "levels": [["A"], ["B"]],
        "round_trips": 2,
    }
    mock_run.return_value = Mock(returncode=0, stdout=json.dumps(response), stderr="")
    mock_machine_config = {
        "hostname": "testhost",
        "venv_path": "/path/to/venv",
    }
    payload = {
        "children_by_job": {"A": ["B"], "B": []},
        "slurm_names": {"A": "A.sh", "B": "B.sh"},
        "slurm_dir": "/remote/slurm",
    }
    result = run_bulk_submit_driver_remotely(payload, machine_config=mock_machine_config)
    assert result == {"A": "1", "B": "2"}


@patch("subprocess.run")
def test_run_bulk_submit_driver_remotely_uses_venv_python_without_bash(mock_run, mock_load_config):
    response = {
        "ok": True,
        "job_ids": {"A": "1"},
        "levels": [["A"]],
        "round_trips": 1,
    }
    mock_run.return_value = Mock(returncode=0, stdout=json.dumps(response), stderr="")
    mock_machine_config = {
        "hostname": "testhost",
        "venv_path": "/remote/venv",
    }
    payload = {
        "children_by_job": {"A": []},
        "slurm_names": {"A": "A.sh"},
        "slurm_dir": "/remote/slurm",
    }
    run_bulk_submit_driver_remotely(payload, machine_config=mock_machine_config)
    command = mock_run.call_args[0][0]
    assert command[0] == "ssh"
    assert "bash -lc" not in " ".join(command)
    assert "/remote/venv/bin/python" in command


@patch("subprocess.run")
def test_run_bulk_submit_driver_remotely_requires_venv_path(mock_run, mock_load_config):
    mock_machine_config = {
        "hostname": "testhost",
        "env_command": "source /path/to/venv/bin/activate",
    }
    payload = {
        "children_by_job": {"A": []},
        "slurm_names": {"A": "A.sh"},
        "slurm_dir": "/remote/slurm",
    }
    with pytest.raises(RuntimeError):
        run_bulk_submit_driver_remotely(payload, machine_config=mock_machine_config)
